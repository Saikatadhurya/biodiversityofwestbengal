...
$edit_id = @$_GET['edit'];
$query = mysqli_query($db,"select * from sub4video where id = '$edit_id'");
...
?>
<form action="editvideo.php?edit_form=<?php echo $edit_id1; ?>" method="POST" enctype="multipart/form-data">
...
<div class="col-sm-3">
				...
				<div class="form-group">
				 <label>Enter Youtube Video id</label>
					...
					<pre>https://www.youtube.com/watch?v=<input class="form-control" name="videoid" type="text" placeholder="VideoID" value="<?php echo $video;?>" required></pre>
					<iframe width="200" height="100 " src="https://www.youtube.com/embed/<?php echo $video; ?>" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
							</div>	
				</div>
	...
	<div class="col-lg-12">			
 <div class="form-group">
       <button type="submit" class="btn btn-primary" name="update">Update Species Video</button>
  </div>
</div>	
...
if(isset($_POST['update']))
{
	...
	else
	{
		...
		$update_query="update sub4video set cname = '$cname', sname = '$sname', video = '$videoid', date = '$date', pname = '$pname', place = '$place', district = '$district', description = '$description' where id = '$update_id'";
		 
		
		if(mysqli_query($db,$update_query))
{
	echo ("<script>alert('Species Video is Successfully Updated')</script>");
	echo"<script>window.open('index.php?editvideo=editvideo','_self')</script>";
}
else{ 
