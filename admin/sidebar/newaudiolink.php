...
<form action="sidebar/newaudiolink.php" method="POST" enctype="multipart/form-data">
<div class="col-lg-15">
...
<div class="col-sm-3">				<div class="form-group">				 <label>Choose Common Name</label>						<select class="form-control selUser" name="name1" onchange="fetch_select3(this.value);"> 						<option value="">Choose Common Name</option>						 <?php							include('../includes/connect.php');							$query = mysqli_query($db,"select * from sub4name order by 1 desc");							while($row1 = mysqli_fetch_assoc($query))							{							$id = $row1['id'];														$cname = $row1['cname'];														 ?>						  <option value="<?php echo $cname;?>"><?php echo $cname; ?> </option>						  <?php 							}							?>						</select>				</div>						</div>				<div class="col-sm-3">				<div class="form-group">				 <label>Scientific Name</label>						<select class="form-control round-form" id="sname" name="name2"> 												</select>										</div>				</div>				<div class="col-sm-12">				<h3><b>OR</b></h3>				</div><div class="col-sm-3">				<div class="form-group">				 <label>Choose Scientific Name</label>						<select class="form-control selUser" name="name3" onchange="fetch_select4(this.value);"> 						<option value="">Choose Scientific Name</option>						 <?php							include('../includes/connect.php');														$query = mysqli_query($db,"select * from sub4name order by 1 desc");							while($row1 = mysqli_fetch_assoc($query))							{							$id = $row1['id'];														$sname = $row1['sname'];																					 ?>						  <option value="<?php echo $sname;?>"><?php echo "$sname" ?> </option>						  <?php 							}							?>						</select>										</div>				</div>								<div class="col-sm-3">				<div class="form-group">				 <label>Common Name</label>						<select class="form-control round-form" id="cname" name="name4" > 												</select>										</div>				</div>											<div class="col-sm-12">				<hr>				</div>
			<!--	<div class="col-sm-3">
				...
				<div class="form-group">
				 <label>Enter Xeno-Canto Audio id</label>
					...
					<pre>https://www.xeno-canto.org/<input class="form-control" name="audioid" type="text" placeholder="AudioID" required></pre>
							</div>	
				</div>
		...
		<div class="col-sm-3">
				<div class="form-group">
				 <label>Enter Recorder Name</label>
	...
	<div class="col-lg-12">			
 <div class="form-group">
       <button type="submit" class="btn btn-primary" name="submit">Add Species Audio</button>
  </div>
</div>	
...
if(isset($_POST['submit']))
{
	...
	if($_POST['name2']=="")		{			$sname = $_POST['name3'];		 $sname = mysqli_real_escape_string($db, $sname);		}		else		{			$sname = $_POST['name2'];		 $sname = mysqli_real_escape_string($db, $sname);		}		if($_POST['name1']!="")		{		   $cname = $_POST['name1'];	    $cname = mysqli_real_escape_string($db, $cname);		}	    else if($_POST['name4']!="")			{			$cname = $_POST['name4'];		 $cname = mysqli_real_escape_string($db, $cname);			}		else		{			$cname = $_POST['name1'];		 $cname = mysqli_real_escape_string($db, $cname);			}		if($_POST['name1']!="" AND $_POST['name3']!="")		{			echo("<script>alert('Invalid Input')</script>");		echo"<script>window.open('../index.php?newimage=newimage','_self')</script>";		exit();		}		if($_POST['name2']=="" AND $_POST['name3']=="")		{			echo("<script>alert('Invalid Input')</script>");		echo"<script>window.open('../index.php?newimage=newimage','_self')</script>";		exit();		}
		...
		$query1 = mysqli_query($db,"select * from sub4audioxeno where sname='$sname' order by slno desc");
		...
		$result = mysqli_query($db, $check_query);
		  ...
		  $uniqueid = $check['uniqueid']."A".$id;
		  $uniqueid = mysqli_real_escape_string($db, $uniqueid);
		   $check_query1 = "SELECT * FROM sub4audioxeno WHERE id='$uniqueid'";
		...
		if($sname != null && $cname != null)
			{
			$query = "INSERT INTO sub4audioxeno (id,cname,sname,audio,date,pname,place,district,description,postedby,slno,timestamp) VALUES('$uniqueid','$cname','$sname','$audioid','$date','$pname','$place','$district','$description','$user_name','$id',now())";
		...
		if(mysqli_query($db,$query))
{
	...
	echo ("<script>alert('Species Audio is Successfully Added')</script>");
	echo"<script>window.open('../index.php?newaudiolink=newaudiolink','_self')</script>";
}
else{ 
