...
<form action="sidebar/sub35.php" method="POST" enctype="multipart/form-data">
<div class="col-lg-15">
<div class="col-sm-3">
				<div class="form-group">
				 <label>Enter Subsection3.5 Name</label>
		
					<input class="form-control" name="sub35" type="text" placeholder="Enter Name" required>
							</div>	
				</div>
				<div class="col-sm-3">
				<div class="form-group">
	...
	<div class="col-lg-12">			
 <div class="form-group">
       <button type="submit" class="btn btn-primary" name="submit">Add Subsection35</button>
  </div>
</div>	
...
{
	if($_POST['kingdom']=='' OR $_POST['sub2']=='' OR $_POST['sub1']=='' OR $_POST['sub25']=='' OR $_POST['sub3']=='' OR $_POST['sub35']=='')
		{
		echo("<script>alert('Fill All the Fields.')</script>");
		echo"<script>window.open('../index.php?sub35=sub35','_self')</script>";
	}
	else
	{
		...
		$imagename = $sub35.".jpg" ;
		 $check_query = "SELECT * FROM sub35 WHERE name='$sub35'";
		$result = mysqli_query($db, $check_query);
		  $check = mysqli_fetch_assoc($result);
		  
			if ($check['name'] == $sub35) {
			  echo ("<script>alert('$sub35 Already Exist')</script>");
			  exit();
			  echo"<script>window.open('../index.php?sub35=sub35','_self')</script>";
		...
		if (($_FILES["image"]["type"] == "image/gif") ||
			...
			$query = "INSERT INTO sub35 (name,sub3,sub25,image,description,sub2) VALUES('$sub35','$sub3','$sub25','$imagename','$description','$sub2')";
...
{
	echo ("<script>alert('Subsection3.5 is Successfully Added')</script>");
	echo"<script>window.open('../index.php?sub35=sub35','_self')</script>";
}
else{ 
